...
docker biuld -t gbozsik/multi-worker:latest -t gbozsik/multi-worker:$SHA -f ./worker/Dockerfile ./worker
...
docker push gbozsik/multi-server:$SHA
docker push gbozsik/multi-worker:$SHA

kubectl apply -f k8s
kubectl set image deployments/server-deployment server=gbozsik/multi-server$SHA
kubectl set image deployments/client-deployment client=gbozsik/multi-client$SHA
